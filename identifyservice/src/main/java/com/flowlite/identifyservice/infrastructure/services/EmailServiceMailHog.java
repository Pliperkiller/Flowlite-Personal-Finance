...
/**
 * Implementación del servicio de email para MailHog (desarrollo).
 * Utiliza MailHog como servidor SMTP local para desarrollo y testing.
 */
@Slf4j
@Service("emailServiceMailHog")
@RequiredArgsConstructor
public class EmailServiceMailHog implements EmailService {
    
    private final JavaMailSender javaMailSender;
    
    @Value("${app.email.from:noreply@flowlite.local}")
    ...
    public void sendVerificationEmail(String email, String verificationToken) {
        try {
            ...
            log.info("=== ENVIANDO EMAIL DE VERIFICACIÓN (MAILHOG) ===");
            log.info("Para: {}", email);
            ...
            log.info("Email HTML enviado exitosamente a MailHog para: {}", email);
    ...
    public void sendWelcomeEmail(String email, String username) {
        try {
            ...
            javaMailSender.send(message);
            log.info("Email de bienvenida enviado a MailHog para: {}", email);
